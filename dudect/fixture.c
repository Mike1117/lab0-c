...
static t_context_t *t[NUMBER_TESTS];
...
{
    t_context_t *tt = max_test();
    double max_t = fabs(t_compute(tt));
    double number_traces_max_t = tt->n[0] + tt->n[1];
...
{
    init_dut();
    percentiles = (int64_t *) calloc(NUMBER_PERCENTILES, sizeof(int64_t));
    for (int i = 0; i < NUMBER_TESTS; i++) {
        t[i] = (t_context_t *) calloc(1, sizeof(t_context_t));
        assert(t[i]);
        t_init(t[i]);
    }
}
