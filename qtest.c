...
void q_shuffle(struct list_head *head)
{
    ...
    // int i = 0;
    int rd = 0;
    for (; curr != head; curr = curr->prev) {
        if (size >= 1) {
            rd = rand() % (size);
        }
        size--;
        for (swp = curr; rd > 0; rd--) {
            swp = swp->prev;
        }
        swap(&list_entry(curr, element_t, list)->value,
             &list_entry(swp, element_t, list)->value);
    }
}
...
static void console_init()
{
    ...
    ADD_COMMAND(linuxsort, "Sort queue with Linux Kernel Sort", "");
